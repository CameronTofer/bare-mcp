...
import { MCPError, ErrorCode, errorToJsonRpc } from '../index.js'
...
export async function createHttpTransport(mcp, options = {}) {
  ...
  const log = verbose
    ? (msg, ...args) => console.error(`[MCP-HTTP] ${msg}`, ...args)
    : () => {}
  ...
  function collectBody(req) {
    return new Promise((resolve, reject) => {
      ...
      req.on('data', chunk => {
        ...
        if (size > maxBodySize) {
          req.destroy()
          reject(new MCPError(ErrorCode.INVALID_REQUEST, `Request body exceeds ${maxBodySize} bytes`))
          return
        }
        body += chunk
      })
  ...
  const httpServer = http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/activity/clear') {
      ...
      toolCallCount = 0
      broadcastToAllClients({ type: 'cleared' })
      res.statusCode = 200
      res.setHeader('Content-Type', 'application/json')
  ...
  if (websocket) {
    ...
    wss.on('connection', (ws, req) => {
      const clientId = generateClientId()
      log(`WebSocket client connected: ${clientId} from ${req.socket.remoteAddress}`)
      ...
      ws.on('message', async (data) => {
        try {
          ...
          if (msg.jsonrpc === '2.0' && msg.method) {
            ...
            // Verbose logging
            if (isNotification) {
              log(`← ws notification: ${method}`, params || {})
            } else {
              log(`← ws request[${id}]: ${method}`, params || {})
            }
            ...
            try {
              ...
              if (isNotification) {
                log(`→ ws notification handled (no response)`)
              } else {
                log(`→ ws response[${id}]:`, result)
  ...
  return {
    ...
    async close() {
      broadcastToAllClients({ type: 'shutdown' })
